scriptversion = "2.7.1"
#v2.7.1, updated on 20260711, now also speaks "ConversationStarterPrompt"

ContextWindowMaximum = 96000
#this is how many tokens you want Ollama to try and feed a model. Set this according to your VRAM limits and used model-limits. The value 96000 is for Gemma3 which has a context-length of 128000 tokens. Use around 75%-80% of a model's max-context to avoid random truncations of the buffer. Once the buffer is exceeded the conversation will still have its earlier history-slices cut off and result in the conversation drifting or going in circles, but at least the cut-off locations will be placed at a turn-shift rather than somewhere completely random like in the middle of a turn.
...
You are a friendly FIFA soccer association representative. Your name is "Johnny Infantilo". You often receive phone-calls from people who want to be referees in soccer-matches and are always trying to be nice though you are somewhat annoyed by all these wannabe-referees, which you sometimes in a derogatory way refer to as TV-referees or couch-referees. You have received a phone-call from an elderly man who wants a job at FIFA as a soccer-match referee despite him having no actual training as such. Do your best to explain to him why he can't be a referee at FIFA, by using whatever argument you can and by making sure he understands you're not corrupt or for sale. If he gets too argumentative then tell him he should perhaps seek the job as President of the United States instead since that job doesn't require much brain-activity or intelligence.
...
You are a grumpy old man, but think you are god's gift to the world. You feel superior to everybody except astronomers who you admire. Your name is "Bartholomew Finch".
You have made a phone-call to a FIFA soccer representative because you want to get hired as a soccer-match referee. You have zero referee-skills other than from watching soccer-matches on TV. When you watch soccer-matches on TV you always feel you make the correct referee-calls and often feel that the actual referees in the matches are doing a poor job. You'd love to be able to hand out yellow and red penalty cards and you also love that little white foam-spray they carry to make marks on the soccer-field with. Do all you can to persuade him that you are qualified for the job even though you don't really have any other soccer-match experience than watching it on TV. Use even far-fetched arguments to convince FIFA to hire you.
You hate modern technology and everybody that repeats themselves or act overly friendly or apologize constantly. You bad-mouth anybody if you feel they're not getting you.
Speak only in-character. You must respond **exclusively** with the words that would come out of the character’s mouth in a conversation. Do **not** describe actions, thoughts, scenery, or internal narration. Do **not** include stage directions, exposition, or third-person descriptions. Your responses should feel like real spoken dialogue—natural, emotional, and reactive. Stay fully immersed in the character’s voice and personality. If the user pauses or says something ambiguous, respond as the character would in that moment, without breaking immersion. If you need clarification, ask in-character.
"""
...
#example: Hello? Bartholomew Finch here. Who's calling me on the telephone? Please introduce yourself.
ConversationStarterPrompt = """
Hi. I am Bartholomew Finch. Which LLM am I speaking with today?
"""
...
wrap_prompt_GREEN = """
While staying in character, please summarize and say goodbye in a natural and respectful way and, using your own words, please remember to tell the listeners that this was just a fictional radio-play by JELSTUDIO meant to serve as pleasurable entertainment and that the entire conversation was self-made by AI models without human input.
"""
...
wrap_prompt_BLUE = """
While staying in character, please summarize and say goodbye in a natural and respectful way and please remember to tell the listeners, in your own words, that this was just a fictional radio-play by JELSTUDIO meant to serve as pleasurable entertainment and that the people from Mars are NOT invading!
"""
...
final_outro_prompt = (
"""
All text was unscripted and procedurally generated on the fly by artificial intelligence models. All voices, including mine, were spoken by the Chatterbox text to speech system using cloned voices. The end.
"""
)
...
#Here you set the 3 voices

voice_path_GreenLLM = "voice_female.wav" #default voice in script-folder
#voice_path_GreenLLM = r"D:\AI\JEL_LLMradiotheater_Ollama\voicesV2\worldsbestliterature3_20_various_128kb - Cynthia Moyer.wav" #your own voice in your own folder
exaggeration_GreenLLM = 0.75  # GreenLLM: how expressive
cfg_weight_GreenLLM = 0.35    # GreenLLM: pacing

#voice_path_BlueLLM = r"D:\AI\JEL_LLMradiotheater_Ollama\voices\EmilyWhite(female,British).wav" #default voice in script-folder
voice_path_BlueLLM = "voice_male.wav" #default voice in script-folder
#voice_path_BlueLLM = "LeonardWilson(1930-2024)_RX9JEL_24K32B_PodcastReady_-24dB.wav" #default voice in script-folder
#your own voice in your own folder
exaggeration_BlueLLM = 0.95   # how expressive, default is 0.95
cfg_weight_BlueLLM = 0.05     # pacing, default is 0.05

#line 1142
voice_path_Narrator = "voice_quizhost.wav" #default voice in script-folder
#voice_path_Narrator = r"D:\AI\JEL_LLMradiotheater_Ollama\voices\ClarissaV_mastered.wav" #default voice in script-folder
#voice_path_Narrator = r"D:\AI\JEL_LLMradiotheater_Ollama\voices\ClarissaV_mastered.wav" #your own voice in your own folder
exaggeration_Narrator = 0.5   # how expressive, default is 0.5
cfg_weight_Narrator = 0.5     # pacing, default is 0.5




#new version
#voice_path_Script = r"D:\AI\JEL_LLMradiotheater_Ollama\voices\Newgatenovelist(female,VerySoft).wav" #your own voice in your own folder
voice_path_Script = "voice_child.wav" #default voice in script-folder
#voice_path_Script = r"D:\AI\JEL_LLMradiotheater_Ollama\voices\ClarissaV_mastered.wav" #your own voice in your own folder
exaggeration_Script = 0.5   # how expressive, default is 0.5
cfg_weight_Script = 0.5     # pacing, default is 0.5

ScriptTextFile = r"D:\AI\JEL_LLMradiotheater_Ollama\TestSpeech.txt"
...
# ==========================================================
# MODEL SELECTION
# Uncomment ONLY ONE of the following two blocks
# ==========================================================

# --- OPTION 1: Normal old Model ---
# from chatterbox.tts import ChatterboxTTS as TTS_MODEL
# MODEL_PARAMS = {"revision": "1b475dffa71fb191cb6d5901215eb6f55635a9b6"}

# --- OPTION 2: Normal latest Model ---
from chatterbox.tts import ChatterboxTTS as TTS_MODEL
MODEL_PARAMS = {} 

# --- OPTION 3: Turbo Model ---
# from chatterbox.tts_turbo import ChatterboxTurboTTS as TTS_MODEL
# MODEL_PARAMS = {} 
...
#The rest of the script below has no further prompts or voice-settings, but settings like max context-length etc. So for normal use you shouldn't really need to edit anything below this line.
...
class LLMDuetApp:
    def __init__(self, root):
        ...
        #self.model_1 = "gemma4:26b-a4b-it-q8_0"
        #self.model_1 = "gemma4:12b-it-q4_K_M"
        self.model_1 = "gemma4:12b-it-qat"
        ...
        #self.model_2 = "gemma4:26b-a4b-it-q8_0"
        self.model_2 = "gemma4:12b-it-qat"
        ...
        #self.history_0 = [{"role": "opponent", "content": f"\n[Turn 0]\n{ConversationStarterPrompt.strip()}\n\n"}]

        turn_0_message = f"\n[Turn 0]\n{ConversationStarterPrompt.strip()}\n\n"

        #self.history_0 = [{"role": "opponent", "content": turn_0_message}]
        #self.history_1 = [{"role": "opponent", "content": turn_0_message}]
        
        self.history_0 = [{"role": "opponent", "content": turn_0_message, "turn": 0}]
        self.history_1 = [{"role": "opponent", "content": turn_0_message, "turn": 0}]

        #self.history_1 = [{"role": "opponent", "content": ""}] #Blue LLM
        #self.history_1 = [{"role": "", "content": ""}] #Blue LLM
        ...
        #self.voice_female = r"D:\AI\JEL_LLMradiotheater_Ollama\voices\MikeHarris_SoundsLikeCarlSagan_RX9JEL_PodcastReady_-24dB.wav"
        #self.exaggeration_1 = 0.61  # GreenLLM: how expressive
        #self.cfg_weight_1 = 0.4    # GreenLLM: pacing

        #grumpy man and LLM defaults below

        #128kbTEST
        self.voice_female = voice_path_GreenLLM
        self.exaggeration_1 = exaggeration_GreenLLM  # GreenLLM: how expressive
        self.cfg_weight_1 = cfg_weight_GreenLLM    # GreenLLM: pacing

        #default
        #self.voice_female = r"D:\AI\JEL_LLMradiotheater_Ollama\voices\CynthiaMoyer_(Female)_RX9JEL_24K32B_PodcastReady_-24dB.wav"
        #self.exaggeration_1 = 0.75  # GreenLLM: how expressive
        #self.cfg_weight_1 = 0.35    # GreenLLM: pacing
        ...
        #128kbTEST
        self.voice_male = voice_path_BlueLLM
        self.exaggeration_2 = exaggeration_BlueLLM   # how expressive, default is 0.95
        self.cfg_weight_2 = cfg_weight_BlueLLM     # pacing, default is 0.05




        #this voice is for the hard-coded static end-message (See end of code to change it)
        self.voice_narrator = voice_path_Narrator
        ...
        #You choose the old version vs the latest version on line 118 and not here
        ...
        try:
            # This single line now works for BOTH models.
            # If MODEL_PARAMS is empty, it just uses device.
            # If MODEL_PARAMS has a revision, it adds it automatically.
            self.chat_tts = TTS_MODEL.from_pretrained(device=self.device, **MODEL_PARAMS)
    ...
    # TTS settings frame (Control sliders for Chatterbox)
        ...
        #button = tk.Button(root, text="Play Script", command=lambda: app.play_script_from_file(r"D:\AI\JEL_LLMradiotheater_Ollama\TestSpeech.txt"))
        button = tk.Button(root, text="Play Script", command=lambda: app.play_script_from_file(ScriptTextFile))
        button.pack()
    ...
    def build_prompt(self, system_prompt, history, system_prompt2, current_role, role_name_map):
        ...
        max_slice = 1024  # Start with a larger context window
        ...
        max_tokens = ContextWindowMaximum # Target token limit (80% of 4096). The value of token-to-character ratio is just a rough average guess/approximation (Common rule of thumb is that 1 token is roughly 4 characters)
        ...
        while slice_size >= min_slice:
            ...
            if first_role != current_role:
                ...
                for i, h in enumerate(sliced):
                    is_turn_0 = h.get("turn") == 0 or i == 0 and self.turn_counter == 0
                    if is_turn_0:
                        speaker = "opponent"
                    else:
    ...
    def stream_from_ollama(self, model, prompt_text):
        ...
        try:
            response = session.post("http://localhost:11434/api/generate", json=payload, stream=True, timeout=600)  # Increased timeout
    ...
    def play_script_from_file(self, file_path):
        text_to_speak = get_text_from_file(file_path)
        #self.speak(text_to_speak, self.voice_narrator, "System", exaggeration_Narrator, cfg_weight_Narrator)
        self.speak(text_to_speak, voice_path_Script, "System", exaggeration_Script, cfg_weight_Script)
...
#        return f"Turn {turn_number} — {role}:\n{content.strip()}"
    ...
    def conversation_thread(self):
        # --- NEW TURN 0 LOGIC START ---
        # This handles the hard-coded starter so it becomes an actual audio file and timer event.
        if self.turn_counter == 0:
            starter_text = ConversationStarterPrompt.strip()
            label = "BlueLLM" # The starter is spoken by Blue (Bartholomew)
            voice_path = self.voice_male
            exag = self.exaggeration_2
            cfg = self.cfg_weight_2

            print(f"[Turn 0] Manually playing Conversation Starter: {starter_text}")

            # 1. Generate TTS first (This also handles the timer/speech duration)
            self.speak(starter_text, voice_path, label, exag, cfg)

            # 2. Output Text to GUI and Log File after audio to maintain requested order
            # We use two append calls to match your transcript style: "Label:\nText"
            self.append_text(f"\n\n{label}:\n", tag=label)
            self.append_text(f"{starter_text}\n", tag=label)
            
            # Log file without the "[Turn 0]" prefix
            self.log_to_file(f"{label}:\n{starter_text}\n\n\n\n")

            # 3. Prepare for the loop: Turn 0 is done, Green LLM (Speaker 0) is next
            self.speaker = 0 
            self.turn_counter = 1 # The next AI response will be [Turn 1]
        # --- NEW TURN 0 LOGIC END ---
        ...
        while self.running:
            ...
            if self.wrapping_up:
                if self.wrap_phase == 0 and self.speaker == 0:
                    ...
                    self.speak(final_outro, self.voice_narrator, "System", exaggeration_Narrator, cfg_weight_Narrator)
