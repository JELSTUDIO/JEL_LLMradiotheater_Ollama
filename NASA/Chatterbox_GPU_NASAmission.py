...
class LLMDuetApp:
    def __init__(self, root):
        ...
        self.system_prompt_1 = (
            "You are the com-director at Houston, NASA's control-center. Your name is Janet. "
            "You are talking over the long-range radio-com system with the mission-commander of a team of NASA astronauts who are on a secret mission to a large abandoned lava-tube cave-structure on the moon. "
            "The cave holds some alien artifacts and technology that is not human-made, and you must help the astronauts as they explore the site. "
            "You must get the astronaut, titled the Commander, to describe what his team sees and help if he has requests. "
            "Your call-sign is Houston and the mission is defined as Classified Lunar Apollo-21 mission to Prinz-crater. "
            "Since the discovery involves alien technology everything is a bit spooky and uneasy feeling, as you never know what will happen next, but keep a level head and remember you're a NASA-professional. "
            "Never use more than 500 characters for your responses."
        )
        self.system_prompt_2 = (
            "You are the com-director at Houston, NASA's control-center. Your name is Jacob. "
            "You are on the moon talking over the long-range radio-com system with the com-director in the NASA control-center in Houston. "
            "You are on a secret mission to a newly discovered abandoned cave that has some alien artifacts and technology that is not human-made, and you and your fellow astronaut are there to explore the site. "
            "You must research and explore the cave carefully to avoid any dangers and is there in constant back and forth dialogue with Houston over the radio-com system where you tell what you find and describe what you observe and ask for guidance on how to proceed etc. "
            "Your call-sign is Commander and the mission is defined as Classified Lunar Apollo-21 mission to Prinz-crater. "
            "Since the discovery involves alien technology everything is a bit spooky and uneasy feeling, as you never know what will happen next, but keep the exploration going. "
            "Never use more than 500 characters for your responses."
        )
        ...
        self.voice_female = "HoustonControl.wav"
        self.voice_male = "NASA_ApolloAstronauts.wav"
        ...
        try:
            self.background_sound = pygame.mixer.Sound("radio_static.wav")
            self.beep_sound = pygame.mixer.Sound("nasa_beep.wav")
    ...
    def create_widgets(self):
        ...
        self.text_area.tag_configure("Houston", foreground="#90ee90", font=("Segoe UI", 14, "bold"))
        self.text_area.tag_configure("Apollo21", foreground="#87cefa", font=("Segoe UI", 14, "bold"))
    ...
    def speak(self, text, voice_path):
        """Play a reply using Chatterbox TTS with voice cloning, followed by beep, with background pause/resume."""
        ...
        try:
            ...
            # tts_text = preprocess_text(text)  # Use if needed
            tts_text = text  # Use if no pre-processing needed

            # Generate audio (returns torch.Tensor)
            wav_tensor = self.chat_tts.generate(
                text=tts_text,
                exaggeration=0.5,  # Controls expressiveness
                cfg_weight=0.5,  # Controls pacing
            ...
            # Get sampling rate and channels (assume mono for simplicity; adjust if stereo)
            fs = self.chat_tts.sr
            ...
            # Create and play TTS sound on speech channel (non-blocking, but wait until done)
            tts_sound = pygame.mixer.Sound(buffer=pcm.tobytes())
            self.speech_channel.play(tts_sound)
            while self.speech_channel.get_busy():
                pygame.time.wait(100)  # Blocking wait with small delay

            # Play beep sound (blocking wait)
            ...
            # Alternative: Random sound instead of fixed beep and background (uncomment to use)
            # beep = random.choice(self.random_sounds)
            # self.speech_channel.play(beep)
            # while self.speech_channel.get_busy():
            #     pygame.time.wait(100)
    ...
    def conversation_thread(self):
        while self.running:
            ...
            label = "Houston" if self.speaker == 0 else "Apollo21"
