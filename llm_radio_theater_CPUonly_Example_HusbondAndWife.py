...
class LLMDuetApp:
    ...
    def create_widgets(self):
        ...
        self.text_area = scrolledtext.ScrolledText(
            self.root, wrap=tk.WORD, state='normal', font=("Consolas", 11),
            bg="#1e1e1e", fg="#dcdcdc", insertbackground="white"
        )
        self.text_area.pack(expand=True, fill=tk.BOTH, padx=10, pady=10)
        self.text_area.tag_configure("Woman", foreground="#90ee90", font=("Consolas", 11, "bold"))
        self.text_area.tag_configure("Man", foreground="#87cefa", font=("Consolas", 11, "bold"))
        self.text_area.tag_configure("error", foreground="#ff6b6b", font=("Consolas", 11, "bold"))
    ...
    def build_prompt(self, system_prompt, history):
        ...
        prompt += " Refine and update your unique perspective based on the new information. Agree if it makes sense to agree, otherwise disagree and make a stronger case to persuade the other of its incorrect view on the subject. Provide a concise, but thoughtful, novel response that advances the conversation without repeating 'Assistant:'.\n\n"
        for h in history[-5:]:
            role = h["role"]
            content = h["content"]
            if role == "user":
