...
#version2.3.6, Chatterbox multilingual version
...
class LLMDuetApp:
    def __init__(self, root):
        self.root = root
        self.root.title("LLM Radio Theater / version2.3.6 / 20250905")
    ...
    def build_prompt(self, system_prompt, history):
        ...
        max_slice = 128  # Start with a larger context window
        min_slice = 3   # Minimum context to retain
        #max_tokens = 1600  # Target token limit (80% of 2048). This is the default used in v2.3.0
        max_tokens = 13000 # Target token limit (75% of 4096). The value is characters, so 16384 characters is estimated to be 4096 tokens, thus 13000 is chosen. Adjust this to suit your own setup.
        #Common rule of thumb is that 1 token is roughly 0.75 of a word. Calculation: 4096 tokens * 0.75 words/token = 3072 standard English words.
